...
"""StoreAverage implements storing information about cars averages in the database"""
...
class StoreAverage:
    ...
    def find_average_by_vin(self, vin: str):
        ...
        return self.__db_session.query(models.Average).filter(models.Average.vin == vin).first()

    def calculate_average(self, model):
        """Calculate average for given model"""
        return self.__db_session.query(func.avg(model.chargingPower).
    ...
    def store_model(self, result):
        ...
        if result is not None:
            ...
            if not self.find_average_by_vin(vin=db_avg_model.vin):
                self.__db_session.add(db_avg_model)
                self.__db_session.commit()
                self.__db_session.refresh(db_avg_model)
                return db_avg_model
            return None
