...
class QueryAverage:
    ...
    def query_average_charging_power(self, vin: str):
        ...
        """
        return self._db_session.query(models.Average.id, models.Average.vin, models.Average.average) \
    ...
    def get_average(self, car: str) -> list:
        ...
        response_list = []
        if car == 'all' or car == '':
            avg_porsche = self.query_average_charging_power(vin=self._cars_vin_list.get('porsche'))
            avg_audi = self.query_average_charging_power(vin=self._cars_vin_list.get('audi'))
            avg_tesla = self.query_average_charging_power(vin=self._cars_vin_list.get('tesla'))
        ...
        elif car == 'porsche' or car == self._cars_vin_list.get('porsche'):
            avg_porsche = self.query_average_charging_power(vin=self._cars_vin_list.get('porsche'))
            response_list.append(self.create_model_if_not_none(avg_porsche))

        elif car == 'audi' or car == self._cars_vin_list.get('audi'):
            avg_audi = self.query_average_charging_power(vin=self._cars_vin_list.get('audi'))
            response_list.append(self.create_model_if_not_none(avg_audi))

        elif car == 'tesla' or car == self._cars_vin_list.get('tesla'):
            avg_tesla = self.query_average_charging_power(vin=self._cars_vin_list.get('tesla'))
            response_list.append(self.create_model_if_not_none(avg_tesla))
